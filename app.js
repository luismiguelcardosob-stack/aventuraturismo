...
window.openNewTabModal=function(){
  ...
  openModal('Nova comanda',`<div class="form-grid"><label>Número / identificação<input id="newTabNumber" placeholder="Ex.: 18"></label><label>Cliente / passageiro<input id="newTabCustomer" placeholder="Opcional"></label></div><div class="checkout"><button class="primary" onclick="createTab()">Criar comanda</button></div>`);
};
...
window.createTab=function(){
  ...
  state.tabs.push({id:id(),number,customer:document.getElementById('newTabCustomer').value.trim(),status:'ABERTA',createdAt:new Date().toISOString(),closedAt:null,total:0});
...
function renderAll(){
  ...
  const open=state.tabs.filter(t=>t.status==='ABERTA'),closed=state.tabs.filter(t=>t.status==='FECHADA');
